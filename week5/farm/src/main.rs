...
fn get_number_from_queue(number: &mut Arc<Mutex<VecDeque<u32>>>) -> Option<u32>{
    let mut numbers_ref = number.lock().unwrap();
    numbers_ref.pop_front()
}


fn helper(mut num_queue : Arc<Mutex<VecDeque<u32>>>){
    loop {
        if let Some(number) = get_number_from_queue(&mut num_queue){
...
fn main() {
    ...
    let mut num_queue = Arc::new(Mutex::new(get_input_numbers()));
    ...
    for _i in 0..num_threads{
        ...
        let thread_process = thread::spawn(move || {
            helper(num_queue_ref);
        });
        thread_list.push(thread_process);
    }
