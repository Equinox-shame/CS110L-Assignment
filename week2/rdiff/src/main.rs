    ...
    for i in 0..m{
        for j in 0..n{
            if seq1[i] == seq2[j]{
                ...
                let val = std::cmp::max(C.get(i+1, j), C.get(i, j+1)).unwrap();
                C.set(i+1, j+1, val);
            }
        }
    }
...
fn main() {
    ...
    let mut lines1 = read_file_lines(filename1).unwrap();
    let mut lines2 = read_file_lines(filename2).unwrap();
    let i = lines1.len();
    let j = lines2.len();
    let mut C = lcs(&lines1, &lines2);
